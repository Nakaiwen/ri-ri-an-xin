...
(function () {
  ...
  const VISITORS = [
    ...
    },
    {
      ...
      greeting: '海浪一波一波慢慢來，日子也可以悠閒過。願您今天心裡安穩。',
    ...
    },
    {
      ...
      scenes: [
        ...
        },
        {
          ...
          greeting: '我剛剛在一個小小的車站，一邊看地圖、一邊等一班緩慢駛來的火車。想跟您說，慢一點的方式，也可以到想去的地方。',
        ...
        },
        {
          ...
          greeting: '我剛剛收到一封遠方的來信，坐在窗邊細細地讀。想跟您說，有人惦記著、有人捎來問候，是很溫暖的事。',
    ...
    },
    {
      ...
      greeting: '我一路晃呀晃地走來，想跟您說：今天不用急，靜靜過也很好。就算只是輕輕鬆鬆地過一天，也很值得。',
  ...
  function clearAllData() {
    ...
    // V2.0: 今日安心籤（清完當天可以重搖）
    localStorage.removeItem(KEYS.qianDrawn);
  ...
  // ============================================================
  // V2.0: 今日安心籤 — 日期定籤（同一天搖到同一支）
  //   干支由 solar-lunar.js 實算；籤詩、宜、祝福皆為安心語
  // ============================================================

  const QIAN_QUOTES = [
    ['月到天心處，風來水面時', '邵雍', '心靜下來的時候，涼涼的風和月光都會來陪你。'],
    ['一陽初動處，萬物未生時', '邵雍', '好事正在安安靜靜地發芽，有它自己的時間表。'],
    ['君子以厚德載物', '《易經》', '心地寬厚的人，什麼福氣都載得動。'],
    ['積善之家，必有餘慶', '《易經》', '平常的好心好意，都會變成福氣回來找你。'],
    ['日新之謂盛德', '《易經》', '每天有一點點新的開始，就很了不起。'],
    ['謙謙君子，卑以自牧', '《易經》', '客客氣氣過日子的人，路都寬寬的。'],
    ['風行水上，自然成文', '《易經》', '不用勉強，順順地過，日子自然就漂亮。'],
    ['上善若水', '《道德經》', '像水一樣溫柔隨和，反而最有力量。'],
    ['知足不辱，知止不殆', '《道德經》', '懂得知足的人，日子過得最安穩。'],
    ['重為輕根，靜為躁君', '《道德經》', '沉得住氣、靜靜的，就是最大的本事。'],
    ['合抱之木，生於毫末', '《道德經》', '再大的樹，也是從小小的芽開始的。'],
    ['虛室生白，吉祥止止', '《莊子》', '心裡留一點空，福氣自己會住進來。'],
    ['水靜猶明，而況精神', '《莊子》', '心像水一樣靜下來，什麼都看得清清楚楚。'],
    ['天地有大美而不言', '《莊子》', '美好的事常常不出聲，就在你身邊。'],
    ['安時而處順，哀樂不能入', '《莊子》', '順著日子過，心就不會被風吹亂。'],
    ['行到水窮處，坐看雲起時', '王維', '覺得沒路的時候，雲正好慢慢升起來。'],
    ['人閒桂花落，夜靜春山空', '王維', '閒閒的、靜靜的，也是很好的日子。'],
    ['明月松間照，清泉石上流', '王維', '安安靜靜的美好，一直都在那裡。'],
    ['采菊東籬下，悠然見南山', '陶淵明', '慢慢過日子，抬頭就有好風景。'],
    ['久在樊籠裡，復得返自然', '陶淵明', '放鬆下來的那種舒服，身體都記得。'],
    ['春有百花秋有月，夏有涼風冬有雪', '無門慧開', '每個季節都有它的好，今天也有今天的好。'],
    ['日日是好日', '禪語', '只要心安，天天都是好日子。'],
    ['平常心是道', '禪語', '平平常常的心，就是最好的日子。'],
    ['雲在青天水在瓶', '李翱', '各自在各自的位置，就很自在。'],
    ['此心安處是吾鄉', '蘇軾', '心安下來的地方，就是家。'],
    ['人間有味是清歡', '蘇軾', '平平淡淡的日子，其實最有滋味。'],
    ['回首向來蕭瑟處，也無風雨也無晴', '蘇軾', '走過的路回頭看，風雨都變成了平常。'],
    ['綠螘新醅酒，紅泥小火爐', '白居易', '一點小小的溫暖，就足夠好好過一個晚上。'],
    ['春眠不覺曉，處處聞啼鳥', '孟浩然', '睡得安穩，醒來有鳥叫，就是好日子。'],
    ['山重水複疑無路，柳暗花明又一村', '陸游', '轉個彎，常常就有新的風景。'],
    ['小荷才露尖尖角', '楊萬里', '小小的新芽，也有它自己慢慢長的時間。'],
    ['清風朗月不用一錢買', '李白', '最好的東西，往往都是不用錢的。'],
    ['偷得浮生半日閒', '李涉', '忙裡偷來的半日清閒，最是珍貴。'],
    ['山中無曆日，寒盡不知年', '太上隱者', '不趕時間的日子，過起來最悠長。'],
    ['德不孤，必有鄰', '《論語》', '心地好的人，身邊自然有人陪。'],
    ['智者樂水，仁者樂山', '《論語》', '喜歡山的、喜歡水的，都各有各的福氣。'],
    ['歲月本長，而忙者自促', '《菜根譚》', '日子其實很寬很長，不趕的人最富有。'],
    ['風恬浪靜中，見人生之真境', '《菜根譚》', '平靜的日子裡，藏著人生最真的味道。'],
    ['寵辱不驚，閒看庭前花開花落', '《小窗幽記》', '心穩穩的，花開花落都是風景。'],
    ['行看流水坐看雲', '禪語', '看看流水、看看雲，一天就很圓滿。']
  ];

  // V2.0.5: 四季籤（第 4 欄為季節：spring/summer/autumn/winter）
  //   抽籤時只有「當令季節」的會加入池子，通用籤則任何時候都可抽
  const QIAN_SEASONAL = [
    // 春 3-5 月
    ['等閒識得東風面，萬紫千紅總是春', '朱熹', 'spring', '春天一到，處處都是繽紛的顏色，日子也跟著亮起來。'],
    ['好雨知時節，當春乃發生', '杜甫', 'spring', '好事會挑對的時候來，就像春雨懂得什麼時候該下。'],
    ['遲日江山麗，春風花草香', '杜甫', 'spring', '春天的陽光暖暖的，草木都香香的，很適合出去走走。'],
    ['竹外桃花三兩枝，春江水暖鴨先知', '蘇軾', 'spring', '春天悄悄來了，最先感覺到暖意的，往往是願意動一動的人。'],
    ['勝日尋芳泗水濱，無邊光景一時新', '朱熹', 'spring', '天氣好的日子出門走走，眼前的風景都像換了新的。'],
    ['細雨魚兒出，微風燕子斜', '杜甫', 'spring', '細細的春雨、輕輕的風，小生命都出來透氣了。'],
    ['春色滿園關不住，一枝紅杏出牆來', '葉紹翁', 'spring', '好的生機是關不住的，總會找到地方冒出頭來。'],
    ['幾處早鶯爭暖樹，誰家新燕啄春泥', '白居易', 'spring', '春天的小鳥忙著找暖枝、銜新泥，新的一年也這樣慢慢起頭。'],
    // 夏 6-8 月
    ['接天蓮葉無窮碧，映日荷花別樣紅', '楊萬里', 'summer', '夏天的荷花開得又大又紅，日子也可以過得這樣飽滿。'],
    ['小荷才露尖尖角，早有蜻蜓立上頭', '楊萬里', 'summer', '剛冒出頭的小荷葉，就有蜻蜓來作伴，好事自有人惦記。'],
    ['綠樹陰濃夏日長，樓臺倒影入池塘', '高駢', 'summer', '夏天的樹蔭濃濃的、白日長長的，正好慢慢過、慢慢歇。'],
    ['連雨不知春去，一晴方覺夏深', '范成大', 'summer', '日子在不知不覺中走著，天一放晴才發現，夏天已經深了。'],
    ['泉眼無聲惜細流，樹陰照水愛晴柔', '楊萬里', 'summer', '泉水靜靜地流、樹蔭輕輕地照，夏天也有它溫柔的一面。'],
    ['水晶簾動微風起，滿架薔薇一院香', '高駢', 'summer', '微風一吹，滿院的花香都來了，涼爽就是夏天的小禮物。'],
    ['稻花香裡說豐年，聽取蛙聲一片', '辛棄疾', 'summer', '稻花香裡有豐收的預感，蛙聲一片，都是好日子的聲音。'],
    ['深居俯夾城，春去夏猶清', '李商隱', 'summer', '春天過去了，夏天剛開始時還很清爽，每個時節都有它的好。'],
    // 秋 9-11 月
    ['自古逢秋悲寂寥，我言秋日勝春朝', '劉禹錫', 'autumn', '別人說秋天冷清，我倒覺得秋天比春天還要好。'],
    ['晴空一鶴排雲上，便引詩情到碧霄', '劉禹錫', 'autumn', '秋高氣爽，抬頭看見一隻鶴飛過，心情也跟著開闊起來。'],
    ['空山新雨後，天氣晚來秋', '王維', 'autumn', '秋雨過後的山特別乾淨，涼涼的傍晚，最適合靜下來。'],
    ['停車坐愛楓林晚，霜葉紅於二月花', '杜牧', 'autumn', '秋天的紅葉，比春天的花還要美，年紀的風景也是這樣。'],
    ['荷盡已無擎雨蓋，菊殘猶有傲霜枝', '蘇軾', 'autumn', '就算花葉凋了，菊枝仍挺著霜站著，人也可以這樣有精神。'],
    ['湖光秋月兩相和，潭面無風鏡未磨', '劉禹錫', 'autumn', '秋天的湖水和月光靜靜相映，心平了，什麼都清清楚楚。'],
    ['桂花香裡覺秋深', '無名氏', 'autumn', '聞到桂花香，就知道秋天深了，日子也該慢慢收一收。'],
    ['稻穗黃時秋正好', '無名氏', 'autumn', '稻穗轉黃的時候，正是一年裡最踏實飽滿的季節。'],
    // 冬 12-2 月
    ['牆角數枝梅，凌寒獨自開', '王安石', 'winter', '牆角的梅花，天再冷也自己開，安安靜靜地美著。'],
    ['遙知不是雪，為有暗香來', '王安石', 'winter', '遠遠看以為是雪，原來是梅花的香先飄過來了。'],
    ['晚來天欲雪，能飲一杯無', '白居易', 'winter', '天要下雪的傍晚，找個人喝杯熱的，就是冬天的溫暖。'],
    ['千山鳥飛絕，萬徑人蹤滅', '柳宗元', 'winter', '冬天的安靜有它的乾淨，一個人靜靜待著也很好。'],
    ['孤舟蓑笠翁，獨釣寒江雪', '柳宗元', 'winter', '一個人守著自己的節奏，再冷的天也自有一份定。'],
    ['忽如一夜春風來，千樹萬樹梨花開', '岑參', 'winter', '一夜大雪像滿樹梨花，冷天裡也藏著意想不到的美。'],
    ['寒夜客來茶當酒，竹爐湯沸火初紅', '杜耒', 'winter', '寒夜有客人來，泡壺熱茶當酒，爐火剛紅，心就暖了。'],
    ['歲寒，然後知松柏之後凋也', '《論語》', 'winter', '天冷了才看得出松柏的堅韌，日子難的時候更見一個人的定。']
  ];

  // 以日期字串做種子的穩定亂數（同一天永遠同一支籤）
  function qianSeededRng(key) {
    let a = 0;
    for (let i = 0; i < key.length; i++) { a = (a * 31 + key.charCodeAt(i)) >>> 0; }
    return function () {
  ...
  // V2.0.5: 依月份判斷當令季節（台灣體感：春3-5 夏6-8 秋9-11 冬12-2）
  ...
  function qianOfToday() {
    // V2.0.5: 抽籤池 = 通用籤 + 當令季節籤
    //   季節籤只在當令時加入，避免夏天抽到冬天的句子
    const season = currentSeason();
    const seasonal = QIAN_SEASONAL
      .filter(function (q) { return q[2] === season; })
      .map(function (q) { return [q[0], q[1], q[3]]; }); // 轉成 [句, 出處, 白話] 三欄格式
    const pool = QIAN_QUOTES.concat(seasonal);
  ...
  // 農民曆式的今日干支（引擎缺席或超出 1930–2050 時安靜略過）
  function qianCalendarLine() {
    try {
      if (typeof solarLunar === 'undefined') return '';
      const d = new Date();
      const y = d.getFullYear();
      if (y < 1930 || y > 2050) return '';
      const L = solarLunar.solar2lunar(y, d.getMonth() + 1, d.getDate(), 12, 0);
      const term = L.getLastTerm();
      return L.getYearInGanZhi() + '年 · ' + L.getDayInGanZhi() + '日' + (term ? ' · 節氣：' + term : '');
    } catch (e) { return ''; }
  }

  function qianDrawnToday() {
    try { return localStorage.getItem(KEYS.qianDrawn) === todayKey(); } catch (e) { return false; }
  }
